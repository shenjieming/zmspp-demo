...
const services = getServices({
  getCustomerListData: { url: '/certificate/register/push/supplier/list/logistics', type: 'post' },
  customerDetail: '/contacts/supplier/relation/finish/org', // 供应商详情
  getCustomerNo: '/certificate/customer/register/push/statistics/logistics',
  getReviewListData: '/certificate/customer/register/push/list/second-review', // 待审核证件列表
  getRefusedListData: '/certificate/customer/register/push/list/refused/logistics', // 已拒绝证件列表
  refusedReasonList: '/system/dicValue/dicKey', // 拒绝原因
  getCertificateDetail: '/certificate/register/push/detail', // 获取审核证件列表
  setPastData: '/certificate/customer/second-receive/certificate', // 审核通过
  setRefusedData: '/certificate/register/push/status/second-refuse', // 审核拒绝
})
const initState = {
...
export default modelExtend({
  namespace: 'recheckCertificateAuditHQ',
  state: initState,
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname, search }) => {
        if (pathname === '/newCredentials/recheckCertificateAuditHQ') {
  ...
  effects: {
    ...
    * getCustomerDetail({ payload }, { select, call, update }) {
      const { selectedCustomer } = yield select(({ recheckCertificateAuditHQ }) => recheckCertificateAuditHQ)
