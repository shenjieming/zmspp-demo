...
export default modelExtend({
  namespace: 'recheckCertificateAudit',
  state: initState,
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname, search }) => {
        if (pathname === '/newCredentials/recheckCertificateAudit') {
  ...
  effects: {
    ...
    * getCustomerDetail({ payload }, { select, call, update }) {
      const { selectedCustomer } = yield select(({ recheckCertificateAudit }) => recheckCertificateAudit)
