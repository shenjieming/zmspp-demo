...
export default modelExtend({
  namespace: 'recheckCertificateAuditZZ',
  state: initState,
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname, search }) => {
        if (pathname === '/newCredentials/recheckCertificateAuditZZ') {
  ...
  effects: {
    ...
    * getCustomerDetail({ payload }, { select, call, update }) {
      const { selectedCustomer } = yield select(({ recheckCertificateAuditZZ }) => recheckCertificateAuditZZ)
