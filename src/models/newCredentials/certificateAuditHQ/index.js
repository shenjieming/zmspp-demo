...
export default modelExtend({
  namespace: 'newCertificateAuditHQ',
  state: initState,
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname, search }) => {
        if (pathname === '/newCredentials/newCertificateAuditHQ') {
  ...
  effects: {
    ...
    * getCustomerDetail({ payload }, { select, call, update }) {
      const { selectedCustomer } = yield select(({ newCertificateAuditHQ }) => newCertificateAuditHQ)
