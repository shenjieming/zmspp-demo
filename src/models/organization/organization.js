...
export default modelExtend({
  ...
  effects: {
    ...
    * queryTableList({ payload }, { call, select, update }) {
      const { searchParam, pagination } = yield select(({ organization }) => organization)
      const { content: { data, current, total, pageSize } } = yield call(getOrgList, {
        ...searchParam,
        ...pagination,
        ...payload,
      })
    ...
    * queryParentOrgList({ payload }, { select, call, update }) {
      const { selectOrg } = yield select(({ organization }) => organization)
    ...
    * saveAddOrg({ payload }, { call, select, update }) {
      const { searchParam } = yield select(({ organization }) => organization)
