...
} from '../../../services/credentials/customerCertificate'
...
export default dvaModelExtend({
  namespace: 'customerCertificate',
  state: {},
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname, search }) => {
        const query = parse(search, { ignoreQueryPrefix: true })
        if (pathname === '/credentials/customerCertificate') {
          // 初始化数据
          dispatch({
            type: 'updateState',
            payload: initState,
  ...
  effects: {
    ...
    * getCertificateNum({ payload }, { call, put, select }) {
      ...
      const certificateNum = yield select(({ customerCertificate }) =>
        customerCertificate.certificateNum)
      yield put({
        type: 'updateState',
        payload: {
          certificateNum: {
