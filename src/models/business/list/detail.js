...
export default modelExtend({
  namespace: 'businessPurchaseDetail',
  state: initState,
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname, search }) => {
        const query = qs.parse(search, { ignoreQueryPrefix: true })
        const match = pathToRegexp('/business/list/detail').exec(pathname)
        if (match) {
  ...
  effects: {
    ...
    * getSessionList({ payload }, { select, call, update }) {
      const {
        isPublisher,
        detail: { chanceStatus },
      } = yield select(({ businessPurchaseDetail }) => businessPurchaseDetail)
      let response
      if (isPublisher === 'true') {
