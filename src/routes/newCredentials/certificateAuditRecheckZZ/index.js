...
const propTypes = {
  recheckCertificateAuditZZ: PropTypes.object,
  loading: PropTypes.object,
}
const namespace = 'recheckCertificateAuditZZ'
const CertificatePush = ({ recheckCertificateAuditZZ, loading }) => {
  const {
    tabType,
    reviewSearchData,
    refusedSearchData,
        ...
        <div className="aek-layout-hor">
          <div>
            <Tabs defaultActiveKey={tabType} onChange={tabChange}>
              <Tabs.TabPane
                tab={getTabName('待审核', customerNo.secondReviewNumber || '')}
                key="review"
              />
              {/*<Tabs.TabPane*/}
...
export default connect(({ recheckCertificateAuditZZ, loading }) => ({ recheckCertificateAuditZZ, loading }))(
  CertificatePush,
)
