...
const OtherBarCode = ({
  ...
  const addModalProp = {
    ...
    zIndex: 1100,
    onCancel() {
      dispatch({ type: 'purchase/updateState', payload: { otherCodeVisible: false } })
    },
  }
  // 条码列表
  const columns = [
    ...
    },
    {
      ...
      render: (value, record, index) => (<Button
        ...
        size="small"
        loading={rowSelectData.barcode === record.barcode && !!effects['purchase/delSkuBarcode']}
        onClick={() => {
          dispatch({
            type: 'purchase/delSkuBarcode',
            payload: {
              pscId: record.pscId,
