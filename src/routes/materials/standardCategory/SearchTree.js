...
import { Row, Radio, Input, Tree, Popover, Spin, Icon, Button } from 'antd'
import style from './SearchTree.less'
import { getTreeItem, cloneDeep } from '../../../utils/'
...
const getIdAndIndex = (data, value, parentId, callback) => {
  data.forEach((item, index) => {
    if (item.value === value) {
      let chgIndex = 0
      let addIndex = 0
      if (index === 0 && item.index !== 0) {
        chgIndex = item.index - 1
      } else {
        chgIndex = item.index
      }
      ...
      return callback([parentId, chgIndex, addIndex, item, item.orgId])
    } else if (item.children && item.children.length) {
      return getIdAndIndex(item.children, value, item.value, callback)
    }
  })
}
...
class SearchTree extends React.Component {
  ...
  render() {
    ...
    const onDrop = (info) => {
      ...
      let idAndIndex = []
      getIdAndIndex(gData, dropKey, '-1', (arr) => {
        idAndIndex = arr
      })
      if (info.dropToGap) {
        if (dropPosition !== -1) {
        ...
        seq.parentId = idAndIndex[0] - 0
        seq.index = idAndIndex[1]
      } else {
        // 拖放到某节点上松开
        seq.parentId = dropKey - 0
        seq.index = idAndIndex[2]
      }
      seq.orgId = idAndIndex[4]
      seq.parentName = getTreeItem(gData, 'value', seq.parentId).label
      if (drop) {
          ...
          <Popover
            ...
            content={
              <span className={style.icon}>
                {nodeData.value - 0 === -1 || <Icon
                  type="minus-circle-o"
                  onClick={() => { removeNode(nodeData) }}
                />}
                {(addDisable && nodeData.value - 0 !== -1) || <Icon
                  type="plus-circle-o"
                  onClick={() => {
                    this.setSelecte([String(nodeData.value)]); addChildNode(nodeData)
                  }}
                />}
              </span>
            }
    ...
    const loop = data => data.map((item) => {
      const { wordArr } = getTreeItem(dataList, 'value', item.value)
      let index
    ...
    const addNodeHandler = () => {
      addHandler(this.state.selectedKeys)
    }
    ...
    const onChange = (e) => {
      ...
      const expand = dataList.map(({ value, wordArr }) => {
    ...
    const searchTreeContent = (
      <Row><RadioGroup style={{ marginTop: 10, marginBottom: 10, width: '100%' }} defaultValue="1" onChange={this.updateTreeState}>
    ...
    return (
      <div style={{ width: treeWidth }}>
        <Row>
          <Button style={{ marginRight: '10px' }} type="primary" onClick={addNodeHandler}>添加</Button>
          <Search style={{ width: '100%', maxWidth: 250 }} placeholder={placeholder} onChange={onChange} />
        </Row>
